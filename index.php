...
// Função auxiliar para renderizar views (pode ser movida para um BaseController)
function renderView(string $viewName, array $data = []): void
{
    extract($data);
    $viewPath = __DIR__ . '/app/Views/' . $viewName . '.php'; // Ajuste o caminho para a pasta Views

    if (file_exists($viewPath)) {
        require $viewPath;
    } else {
        // Fallback para erro se a view não for encontrada
...
$primeiroAcesso = $_SESSION['primeiro_acesso'] ?? null;

// Redirecionamento para login ou primeiro acesso se necessário
// Exclui rotas de autenticação para evitar loops de redirecionamento
if (!$isLoggedIn && !in_array($requestUri, ['/login', '/authenticate'])) {
    header("Location: /login?redirect=" . urlencode($_SERVER['REQUEST_URI']));
    exit();
}

// Se logado e for primeiro acesso, força a troca de senha
// Exclui a rota de processamento de primeiro acesso para evitar loops
if ($isLoggedIn && $primeiroAcesso === 1 && $requestUri !== '/first-access' && $requestUri !== '/authenticate-first-access') {
...
switch ($requestUri) {
    ...
    case '/procedimentos/mostrar':
        ...
        if ($id !== null && is_numeric($id)) {
            ...
            // Renderiza 404 se ID inválido ou não fornecido
    ...
    case '/procedimentos/criar':
        ...
        $controller->criar();
        break;
    case '/procedimentos/salvar': // Nova rota para salvar POST
        $controller = new ProcedimentoController();
        $controller->salvar();
        break;
    case '/procedimentos/editar': // Nova rota para exibir formulário de edição
        $controller = new ProcedimentoController();
        $id = $query['id'] ?? null;
        if ($id !== null && is_numeric($id)) {
